...
class Op:
    ...
    def __lt__(self, other):
        ...
        """
        o = BinaryOpReversible(COMPARISON_OPERATOR_PRIORITY, self, other, operator.lt, "<", lambda x: x._rhs > x._lhs)
    ...
    def __le__(self, other):
        o = BinaryOpReversible(COMPARISON_OPERATOR_PRIORITY, self, other, operator.le, "<=", lambda x: x._rhs >= x._lhs)
    ...
    def __gt__(self, other):
        return BinaryOpReversible(COMPARISON_OPERATOR_PRIORITY, self, other, operator.gt, ">", lambda x: x._rhs < x._lhs)

    def __ge__(self, other):
        return BinaryOpReversible(COMPARISON_OPERATOR_PRIORITY, self, other, operator.ge, ">=", lambda x: x._rhs <= x._lhs)
...
class ComprehensionOp(Op):
    ...
    def __repr__(self):
        ...
        f = (
            "{{{} {}}}"
            if self._tp is dict and isinstance(self._item,
                                                           KeyValueOp)
    ...
    def _generate(expr, *args, **kwargs):
        ...
        def gen(remaining_layers):
            ...
            ti = {
                n: i.__invoke__(*args, **kwargs, **{it._name: it for it in iterators.values()}) for n, i in
                  ti.items()
            }
